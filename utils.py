...
def compute_psnr_ssim(recovered, clean):
    assert clean.shape == recovered.shape
    clean = np.clip(clean.detach().cpu().numpy(), 0, 1)
    ...
    for i in range(N):
        ...
        win_size = min(7, min(clean[i].shape[0], clean[i].shape[1]))
        if win_size % 2 == 0:
            win_size -= 1  # 确保 win_size 是奇数
        ssim += structural_similarity(clean[i], recovered[i], data_range=1, win_size=win_size, channel_axis=-1)
